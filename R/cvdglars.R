cvdglars <- function(X,y,family=c("binomial","poisson"),control=list()){
	...
	stop("read the documentation for 'nfold' more carefully")
	if(is.null(setting$foldid)) setting$foldid=sample(n)
	else{
		if(min(setting$foldid)<1 | max(setting$foldid)>n)
		stop("read the documentation for 'foldid' more carefully")
	}
	if(setting$ng<1)
	stop("read the documentation for 'ng' more carefully")
	...
	fit=switch(setting$algorithm,
			   pc=cvdglars_pc(n,p,X,y,family,setting),
			   ccd=cvdglars_ccd(n,p,X,y,family,setting)
			   )
	fit$call <- this.call
	fit$family <- family
