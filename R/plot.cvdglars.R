...
#	g_hat <- g[which.min(dev_m)]
	df <- sum(abs(x$beta)>0)
	if(!x$family$family %in% c("binomial", "poisson")) df <- df + 1
    plot(g, dev_m, xlab = expression(gamma), ylab = "Deviance",
            ylim = c(min(dev_low), max(dev_up)), pch = 20, type = "n",
            main = "Cross-Validation Deviance", ...)
