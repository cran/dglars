subroutine pc_gamma_c(n,p,X,y,nup,w,b,phi,ru,dev,A,nv,nav,nnonzero,g_seq,mthd,g0,g_hat,dg_max,eps,np,ncrct,cf,NReps,nNR,conv)
...
X2 = X*X
nstp = 1
call w_mk_gamma_c(n,p,X,X2,w,conv)
if(conv.eq.5) then
    np=nstp
    return
end if
...
if(nup.ne.0) then
    ...
    ba = 0.d0
    ba(0) = b(0,nstp)
    call bastart_gamma_c(n,nup,X(:,A(1:nup)),X2(:,A(1:nup)),y,NReps,nNR,ba,conv)
    if(conv.eq.5) return
    if(conv.eq.0) then
        b(0,nstp)=ba(0)
        b(A(1:nup),nstp)=ba(1:nup)
        call eta_mk(n,nup,X(:,A(1:nup)),ba,eta)
        call mu_mk_gamma(n,eta,mu,conv)
        if(conv.eq.5) then
            np=nstp
            return
        end if
...
do k=2,np
    ...
    call prd_gamma_c(mthd,g,g0,n,p,X,X2,A,nav,nup,ba,mu,dmu_dth,sqrt_ib,w(1:p),ruv_old,dg_max,dba,dg,conv,ai,final)
    if(conv.ne.0) then
        np=nstp
        return
    end if
    do nsbstp=1,ncrct
        call crct_gamma_c(n,nav,X(:,A(1:nav)),X2(:,A(1:nav)),y,nup,ba,dba,g,dg,w(A(1:nav)),ruv_old(A(1:nav)),&
...
np = nstp
end subroutine pc_gamma_c
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Subroutine used in the starting step
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine bastart_gamma_c(n,nav,Xa,X2a,y,NReps,n_step,ba,conv)
...
end subroutine bastart_gamma_c
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Subroutines used in the prediction step
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine prd_gamma_c(mthd,g,g0,n,p,X,X2,A,nav,nup,ba,mu,dmu_dth,sqrt_ib,w,ruv,dg_max,dba,dg,conv,ai,final)
...
call d2mu_dth2_mk_gamma(n,mu,d2mu_dth2)
...
end subroutine prd_gamma_c
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Subroutines used in the corrector step
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine crct_gamma_c(n,nav,Xa,X2a,y,nup,ba,dba,g,dg,wa,rua,NReps,nNR,mu,dmu_dth,ba_crct,conv)
...
ba_prd=ba-dg*dba
call newt_gamma_c(n,nav,va,Xa,X2a,y,nup,wa,NReps,nNR,mu,dmu_dth,ba_prd,conv)
if(conv.ne.0) return
ba_crct=ba_prd
end subroutine crct_gamma_c
!
subroutine newt_gamma_c(n,nav,va,Xa,X2a,y,nup,wa,NReps,n_step,mu,dmu_dth,ba_crct,conv)
...
end subroutine newt_gamma_c
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Subroutines related to the gamma family
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine mu_mk_gamma(n,eta,mu,check)
integer    ::    n,check
double precision :: eta(n),mu(n)
check=0
mu = -1.d0/eta
if(any(mu.le.0.)) check=5
end subroutine mu_mk_gamma
!
subroutine dmu_dth_mk_gamma(n,mu,dmu_dth)
integer :: n
double precision :: mu(n),dmu_dth(n)
dmu_dth=mu**2
end subroutine dmu_dth_mk_gamma
!
subroutine    d2mu_dth2_mk_gamma(n,mu,d2mu_dth2)
integer    ::    n
double precision    ::    mu(n),d2mu_dth2(n)
d2mu_dth2=2.d0*mu**3
end subroutine    d2mu_dth2_mk_gamma
!
subroutine    d2th_dmu2_mk_gamma(n,mu,d2th_dmu2)
integer    ::    n
double precision    ::    mu(n),d2th_dmu2(n)
d2th_dmu2=-2.d0/mu**3
end subroutine  d2th_dmu2_mk_gamma
!
subroutine deviance_gamma(n,y,mu,dev)
integer    ::    n
double precision    :: y(n),mu(n),dev
dev=2.d0*(sum((y-mu)/mu - log(y/mu)))
end subroutine deviance_gamma
!
subroutine w_mk_gamma_c(n,p,X,X2,w,check)
integer :: n,p,i,check
