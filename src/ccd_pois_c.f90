subroutine ccd_pois_c(n,p,X,y,nup,w,np,g0,g_hat,nstp,eps,NReps,nNR,mthd,b,phi,ru,dev,g_seq,A,nnonzero,nav,conv)
double precision, parameter :: big = 9.9e30, zero = 1.0e-5
integer :: n,p,nup,np,nstp,nNR,mthd,A(0:p),nnonzero(np),nav,conv
double precision :: X(n,p),y(n),w(0:p),g0,g_hat,eps,NReps,b(0:p,np),phi(np),ru(p,np),dev(np),g_seq(np)
...
if(nup .ne. 0) then
    ...
    call bastart_pois_c(n, nup, X(:, A(1:nup)), X2(:, A(1:nup)), y, NReps, nNR, bstart, conv)
    if(conv .eq. 0) then
        ob(A(0:nup)) = bstart
...
do step = 1, np
    ...
    nnonzero(step) = nav + 1
    call deviance_pois(n, y, mu, dev(step))
end do
end subroutine ccd_pois_c
