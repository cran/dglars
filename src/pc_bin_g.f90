subroutine pc_bin_g(linkid,n,p,X,y,mi,nup,w,b,phi,ru,dev,A,nv,nav,nnonzero,g_seq,mthd,g0,g_hat,dg_max,eps,np,ncrct,cf,&
NReps,nNR,conv)
...
double precision :: X(n,p),y(n),mi(n),w(0:p),dev(np),g_seq(np),eps,b(0:p,np),phi(np),ru(p,np),dg_max,g0,g_hat,NReps
double precision :: cf,dg,dg_tmp1,dg_tmp2,g
double precision :: eta(n),mu(n),ruv_old(p),ruv_new(p),dmu_dth(n),dmu_de(n),dth_de(n),sqrt_ib(p),X2(n,p)
...
X2 = X*X
nstp = 1
call w_mk_bin_g(linkid,n,p,mi,X,X2,w,conv)
if(conv.eq.5) then
    np=nstp
    return
end if
...
if(nup.ne.0) then
    ...
    ba=0.
    ba(0)=b(0,nstp)
    call bastart_bin_g(linkid,n,nup,X(:,A(1:nup)),X2(:,A(1:nup)),y,mi,NReps,nNR,ba,conv)
    if(conv.eq.0) then
        b(0,nstp)=ba(0)
        b(A(1:nup),nstp)=ba(1:nup)
        call eta_mk(n,nup,X(:,A(1:nup)),ba,eta)
        call mu_mk(linkid,n,eta,mi,mu)
        if(any(mu.lt.0.d0).or.any(mu.gt.mi)) then
            conv=5
            np=nstp
            return
        end if
...
do k=2,np
    ...
    call prd_bin_g(linkid,mthd,g,g0,n,p,X,X2,y,A,nav,nup,ba,mi,eta,mu,dth_de,dmu_de,dmu_dth,sqrt_ib,&
    ...
    do nsbstp=1,ncrct
        call crct_bin_g(linkid,n,nav,X(:,A(1:nav)),X2(:,A(1:nav)),y,nup,ba,dba,g,dg,w(A(1:nav)),ruv_old(A(1:nav)),&
        ...
        if(conv.ne.0) then
            ...
            if(test(1).or.test(2)) then
                ...
                if(dg.le.dzero) exit
                call mu_mk(linkid,n,eta,mi,mu)
                if(any(mu.lt.0.).or.any(mu.gt.mi)) then
                    conv=5
                    np=nstp
                    return
                end if
...
np = nstp
end subroutine pc_bin_g
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine w_mk_bin_g(linkid,n,p,mi,X,X2,w,check)
integer :: linkid,n,p,i,check
double precision :: X(n,p),X2(n,p),w(0:p),eta(n),mi(n),mu(n),dmu_de(n),dmu_dth(n),wght(n)
if(w(1).ne.0.d0) then
    ...
    w(0) = 1.d0
    forall(i=1:p) w(i)=0.5*sum(wght*X2(:,i))*w(i)**2
else
    w = 1.d0
end if
end subroutine w_mk_bin_g
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Subroutine used in the starting step
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine bastart_bin_g(linkid,n,nav,Xa,X2a,y,mi,NReps,n_step,ba,conv)
...
end subroutine bastart_bin_g
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Subroutines used in the prediction step
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine prd_bin_g(linkid,mthd,g,g0,n,p,X,X2,y,A,nav,nup,ba,mi,eta,mu,dth_de,dmu_de,dmu_dth,sqrt_ib,w,ruv,&
...
call d2th_dmu2_mk_bin(n,mi,mu,d2th_dmu2)
...
end subroutine prd_bin_g
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Subroutines used in the corrector step
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine crct_bin_g(linkid,n,nav,Xa,X2a,y,nup,ba,dba,g,dg,wa,rua,NReps,nNR,mi,eta,mu,dth_de,dmu_de,ba_crct,conv)
...
ba_prd=ba-dg*dba
call newt_bin_g(linkid,n,nav,va,Xa,X2a,y,nup,wa,NReps,nNR,mi,eta,mu,dth_de,dmu_de,ba_prd,conv)
if(conv.ne.0) return
ba_crct=ba_prd
end subroutine crct_bin_g
!
subroutine newt_bin_g(linkid,n,nav,va,Xa,X2a,y,nup,wa,NReps,n_step,mi,eta,mu,dth_de,dmu_de,ba_crct,conv)
...
do    i=1, n_step
    ...
    d2th_de2=dmu_de**2*d2th_dmu2+d2mu_de2/dmu_dth
    wght1=dth_de*dmu_de-d2th_de2*r
    wght2=2.*dth_de*d2mu_de2+dmu_de**3*d2th_dmu2
