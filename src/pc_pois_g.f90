subroutine pc_pois_g(linkid,n,p,X,y,nup,w,b,phi,ru,dev,A,nv,nav,nnonzero,g_seq,mthd,g0,g_hat,dg_max,eps,np,ncrct,cf,NReps,nNR,conv)
...
double precision :: X(n,p),y(n),w(0:p),dev(np),g_seq(np),eps,b(0:p,np),phi(np),ru(p,np),dg_max,g0,g_hat,NReps,cf,dg
double precision :: dg_tmp1,dg_tmp2,g,eta(n),mu(n),ruv_old(p),ruv_new(p),dmu_dth(n),dmu_de(n),dth_de(n),sqrt_ib(p),X2(n,p),mi(n)
...
X2 = X*X
nstp = 1
call w_mk_pois_g(linkid,n,p,mi,X,X2,w,conv)
if(conv.eq.5) then
    np=nstp
    return
end if
...
if(nup.ne.0) then
    ...
    ba=0.d0
    ba(0)=b(0,nstp)
    call bastart_pois_g(linkid,n,nup,X(:,A(1:nup)),X2(:,A(1:nup)),y,mi,NReps,nNR,ba,conv)
    if(conv.eq.0) then
        b(0,nstp)=ba(0)
        b(A(1:nup),nstp)=ba(1:nup)
        call eta_mk(n,nup,X(:,A(1:nup)),ba,eta)
        call mu_mk(linkid,n,eta,mi,mu)
        if(any(mu.lt.0.d0)) then
            conv=5
            np=nstp
            return
        end if
        call deviance_pois(n,y,mu,dev(nstp))
    else
        nup=0
    end if
...
phi(nstp)=1.d0
nav=nup
call dmu_dth_mk_pois(n,mu,dmu_dth)
...
do k=2,np
    ...
    call prd_pois_g(linkid,mthd,g,g0,n,p,X,X2,y,A,nav,nup,ba,mi,eta,mu,dth_de,dmu_de,dmu_dth,sqrt_ib,w(1:p),ruv_old,&
    ...
    do nsbstp=1,ncrct
        call crct_pois_g(linkid,n,nav,X(:,A(1:nav)),X2(:,A(1:nav)),y,nup,ba,dba,g,dg,w(A(1:nav)),ruv_old(A(1:nav)),&
...
np = nstp
end subroutine pc_pois_g
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Subroutines used to compute the weights
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine w_mk_pois_g(linkid,n,p,mi,X,X2,w,check)
...
if(w(1).ne.0.d0) then
    ...
    call dmu_dth_mk_pois(n,mu,dmu_dth)
    call dmu_de_mk(linkid,n,mi,mu,dmu_de)
...
end subroutine w_mk_pois_g
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Subroutines used in the starting step
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine bastart_pois_g(linkid,n,nav,Xa,X2a,y,mi,NReps,n_step,ba,conv)
...
end subroutine bastart_pois_g
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Subroutines used in the prediction step
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine prd_pois_g(linkid,mthd,g,g0,n,p,X,X2,y,A,nav,nup,ba,mi,eta,mu,dth_de,dmu_de,dmu_dth,sqrt_ib,w,&
                    ruv,dg_max,dba,dg,conv,ai,final)
...
call d2th_dmu2_mk_pois(n,mu,d2th_dmu2)
...
end subroutine prd_pois_g
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Subroutines used in the corrector step
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine crct_pois_g(linkid,n,nav,Xa,X2a,y,nup,ba,dba,g,dg,wa,rua,NReps,nNR,mi,eta,mu,dth_de,dmu_de,ba_crct,conv)
integer :: linkid,n,nav,nup,nNR,conv
double precision :: Xa(n,nav),X2a(n,nav),y(n),g,dg,wa(nav),rua(nav),NReps,mi(n),eta(n),mu(n),dth_de(n),dmu_de(n),va(nav)
double precision, dimension(0:nav) :: ba,dba,ba_crct,ba_prd
va=0.d0
va((nup+1):nav)=(g-dg)*sign(1.,real(rua((nup+1):nav)))
ba_prd=ba-dg*dba
call newt_pois_g(linkid,n,nav,va,Xa,X2a,y,nup,wa,NReps,nNR,mi,eta,mu,dth_de,dmu_de,ba_prd,conv)
if(conv.ne.0) return
ba_crct=ba_prd
end subroutine crct_pois_g
!
subroutine newt_pois_g(linkid,n,nav,va,Xa,X2a,y,nup,wa,NReps,n_step,mi,eta,mu,dth_de,dmu_de,ba_crct,conv)
integer    :: linkid,n,nav,nup,n_step,conv,i,j
double precision :: wa(nav),ba_crct(0:nav),va(nav),Xa(n,nav),X2a(n,nav),y(n),NReps,mi(n),eta(n),mu(n),sum_abs_f,sum_abs_db
double precision :: dba(0:nav),r(n),sqrt_i_ba(nav),rua(nav),Drua(0:nav,0:nav)
